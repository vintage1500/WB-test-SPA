<template>
  <div class="container">
    <h2 style="margin:8px 0 12px;">Stocks</h2>
    ...
    <FiltersBar v-model="filters" @apply="applyFilters" @reset="resetFilters">
      <!-- для stocks критичен dateFrom (текущий день) -->
      <label>Поле фильтра
...
const filters = reactive({ dateFrom: '', dateTo: '', q: '' }) // dateTo можно не задавать для stocks
...
async function load() {
  ...
  try {
    const { items: list } = await fetchEndpoint(endpoint, {
      dateFrom: filters.dateFrom || undefined, // ensureDates подставит сегодня, если пусто
